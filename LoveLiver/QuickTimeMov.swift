...
class QuickTimeMov {
    ...
    fileprivate let dummyTimeRange = CMTimeRange(start: CMTime(value: CMTimeValue(exactly: 0)!, timescale: CMTimeScale(exactly: 1000)!), duration: CMTime(value: CMTimeValue(exactly: 200)!, timescale: CMTimeScale(exactly: 3000)!))
    ...
    func readStillImageTime() -> NSNumber? {
        if let track = track(AVMediaType(rawValue: AVMediaType.metadata.rawValue)) {
    ...
    func write(_ dest : String, assetIdentifier : String) {
        do {
            ...
            // --------------------------------------------------
            guard let track = self.track(AVMediaType(rawValue: AVMediaType.video.rawValue)) else {
